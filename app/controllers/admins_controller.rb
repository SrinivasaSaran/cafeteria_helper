class AdminsController < ApplicationController
  ...
  def role_change
    if User.find(@current_user.id).role == "clerk" || User.find(@current_user.id).role == "admin"
      session[:original_order_id] = session_current_order_id
      session[:original_user_id] = @current_user.id
      session[:current_order_id] = session_walkin_order_id
      session[:current_user_id] = Order.find(session_walkin_order_id).user.id
  ...
  def role_back
    if User.find(@original_user.id).role == "clerk" || User.find(@original_user.id).role == "admin"
      session[:walkin_order_id] = session_current_order_id
      session[:current_user_id] = @original_user.id
      session[:current_order_id] = session_original_order_id
